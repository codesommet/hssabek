    ...
    <div>
        <div class="d-flex align-items-center justify-content-between border-bottom flex-wrap row-gap-3 mb-3 pb-3">
            ...
            <div>
                ...
                @if($invoice->due_date)
                <p class="mb-0">Échéance : <span class="text-dark">{{ $invoice->due_date?->format('d/m/Y') }}</span></p>
                @endif
                @if($invoice->reference_number)
                <p class="mb-0">Réf : <span class="text-dark">{{ $invoice->reference_number }}</span></p>
                @endif
            </div>
        </div>
        ...
        <div class="table-responsive border border-bottom-0 mb-3">
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td colspan="2" class="text-dark">
                            <div>
                                <h6 class="fs-14 fw-medium mb-2">Sous-total HT</h6>
                                @if($invoice->discount_total > 0)
                                <h6 class="fs-14 fw-medium mb-2">Remise</h6>
                                @endif
                        ...
                        <td class="text-dark text-end fw-medium">
                            <div>
                                <h6 class="fs-14 fw-medium mb-2">{{ number_format($invoice->subtotal, 2, ',', ' ') }} {{ $currency }}</h6>
                                @if($invoice->discount_total > 0)
                                <h6 class="fs-14 fw-medium mb-2">-{{ number_format($invoice->discount_total, 2, ',', ' ') }} {{ $currency }}</h6>
                                @endif
                                @if($invoice->enable_tax)
                    ...
                    <tr>
                        <td colspan="3" class="text-dark bg-light">Total articles : {{ $invoice->items->count() }}</td>
                        <td colspan="2" class="text-dark fw-medium bg-light"><h6>Total TTC</h6></td>
                        <td class="text-dark text-end fw-medium bg-light"><h6>{{ number_format($invoice->total, 2, ',', ' ') }} {{ $currency }}</h6></td>
                    </tr>
            ...
            <table class="table">
                <tbody>
                    <tr>
                        ...
                        <td colspan="2" class="text-dark fw-medium">Montant payé</td>
                        <td class="text-dark text-end fw-medium">{{ number_format($invoice->amount_paid, 2, ',', ' ') }} {{ $currency }}</td>
                    </tr>
                    <tr>
                        <td colspan="4" class="border-0"></td>
                        <td colspan="2" class="text-dark fw-medium"><h6>Solde dû</h6></td>
                        <td class="text-dark text-end fw-medium"><h6>{{ number_format($invoice->amount_due, 2, ',', ' ') }} {{ $currency }}</h6></td>
                    </tr>
                </tbody>
            </table>
        </div>
