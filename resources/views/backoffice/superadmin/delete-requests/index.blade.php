                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @endphp
                                <span class="badge badge-soft-secondary">{{ $reasonLabels[$request->reason_type] ?? $request->reason_type }}</span>
                                @if($request->reason_details)
                                    <i class="isax isax-info-circle ms-1 text-muted" data-bs-toggle="tooltip" title="{{ $request->reason_details }}"></i>
                                @endif
                            </td>
                            ...
                            <td>
                                @switch($request->status)
                                    @case('pending')
                                        <span class="badge badge-soft-warning d-inline-flex align-items-center">En attente<i class="isax isax-clock ms-1"></i></span>
                                        @break
                                    @case('confirmed')
                                        <span class="badge badge-soft-danger d-inline-flex align-items-center">Confirmé<i class="isax isax-tick-circle ms-1"></i></span>
                                        @break
                                    @case('cancelled')
                                        <span class="badge badge-soft-success d-inline-flex align-items-center">Annulé<i class="isax isax-close-circle ms-1"></i></span>
                                        @break
                                @endswitch
                            </td>
                            <td>
                                @if($request->status === 'pending')
                                    <form method="POST" action="{{ route('sa.delete-requests.cancel', $request) }}" class="d-inline">
                                        @csrf
                                        <button type="submit" class="btn btn-outline-white d-inline-flex align-items-center me-1"
        ...
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content">
                <div class="modal-body text-center">
                    ...
                    <p class="mb-2">Êtes-vous sûr de vouloir confirmer la suppression du compte de <strong>{{ $req->tenant->name ?? '-' }}</strong> ?</p>
